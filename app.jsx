...
function SideMenu({ open, onClose, currentUser, isAdmin, onNavigate, onLogout }) {
  return (
    <>
      <div
        onClick={onClose}
              ...
              >
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <span style={{ fontWeight: 700, fontSize: "1.02rem", color: tokens.cream }}>{p.title}</span>
                    {p.isPrivate && <Icon.Lock color={tokens.amber} />}
                  </div>
                  <div style={{ fontSize: "0.76rem", color: "rgba(217,200,168,0.45)", marginTop: 4 }}>
                    Updated {timeAgo(p.updatedAt)} by {p.updatedBy || "unknown"}
                  </div>
                </div>
                <span style={{ color: tokens.moss2, fontSize: "1.2rem" }}>→</span>
              </button>
            ))}
        </div>
      )}
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, marginBottom: 8 }}>
        <h1 style={{ fontFamily: "'Rubik'", fontWeight: 900, fontSize: "2.1rem", color: tokens.cream, margin: 0 }}>{page.title}</h1>
        {isAdmin && (
...
function LoginView({ onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === "login") {
      ...
      const signUpRes = await supabaseSignUp(email.trim(), password);
      if (!signUpRes.ok) {
      ...
      const insertRes = await dbInsert("profiles", {
        id: userId,
        username: username.trim(),
        role: "viewer",
      });
      if (!insertRes.ok) {
        setError(insertRes.error.includes("duplicate") ? "That username is already taken." : insertRes.error);
        setSubmitting(false);
        return;
      }
  ...
  return (
    <div>
      ...
      >
        ...
        <ToolBtn onClick={() => exec("underline")} label="U" title="Underline" underline />
        <Divider />
        <ToolBtn onClick={() => exec("justifyLeft")} title="Align left" label="⟸" />
        <ToolBtn onClick={() => exec("justifyCenter")} title="Align center" label="↔" />
        <ToolBtn onClick={() => exec("justifyRight")} title="Align right" label="⟹" />
        <Divider />
        <ToolBtn onClick={() => exec("insertUnorderedList")} title="Bullet list" label="•≡" />
        <Divider />
        <ToolBtn onClick={insertLink} title="Insert link" label="🔗" />
        <ToolBtn onClick={insertImage} title="Insert image" label="🖼" />
        <Divider />
...
function ToolBtn({ onClick, label, title, bold, italic, underline }) {
  const [hover, setHover] = useState(false);
  return (
    <button
      onClick={(e) => {
      ...
      style={{
        ...
        height: 32,
        padding: "0 8px",
        background: hover ? "rgba(107,173,92,0.18)" : "rgba(217,200,168,0.05)",
        border: `1px solid ${tokens.border}`,
        borderRadius: 6,
        color: tokens.bone,
        cursor: "pointer",
...
function Divider() {
  return <div style={{ width: 1, height: 22, background: tokens.border, margin: "0 2px" }} />;
}
