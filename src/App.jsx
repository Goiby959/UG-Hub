  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (res.ok && res.data.length > 0) {
        ...
        setCurrentUser({ id: profile.id, username: profile.username, role: profile.role, email: currentSession.user.email, bannedUntil });
      } else {
...
function LoginView({ onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === "login") {
      ...
      onSuccess({ id: profile.id, username: profile.username, role: profile.role, email: signInRes.data.user.email, bannedUntil: profile.banned_until || null });
    } else {
