...
function FanArtPost({ postId, currentUser, isBannedUser, onBack, showToast }) {
  ...
  useEffect(() => {
    (async () => {
      const [postRes, likeRes, allLikesRes, commentRes, boostRes, allBoostsRes, balanceRes] = await Promise.all([
        ...
        dbSelect("fan_art_boosts", `art_id=eq.${postId}&select=id,user_id,created_at,profiles(username)&order=created_at.desc`),
      ...
      setBoostCount(allBoosts.length);
      setBoostsList(allBoosts);
  ...
  const applyBoost = async () => {
    ...
    if (!res.ok) { showToast("Couldn't apply boost", "error"); return; }
    const newEntry = { ...(res.data[0] || {}), profiles: { username: currentUser.username } };
    setBoosted(true);
    setBoostCount((c) => c + 1);
    setBoostsList((prev) => [newEntry, ...prev]);
    if (!isUnlimitedBoosts) {
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 28, flexWrap: "wrap" }}>
        ...
        <button
          ...
          style={{
            ...
            borderColor: boostCount > 0 ? "rgba(91,175,212,0.6)" : (canBoost ? tokens.border : "rgba(217,200,168,0.15)"),
            background: boostCount > 0 ? "rgba(91,175,212,0.12)" : "transparent",
          }}
        >
          <span style={{ fontSize: "1rem", lineHeight: 1 }}>↑</span>
          {boostCount > 0 ? boostCount : "Boost"}
          {!isUnlimitedBoosts && boostsRemaining !== null && boostsRemaining > 0 && (
