...
function FanArtPost({ postId, currentUser, isBannedUser, onBack, showToast }) {
  ...
  const toggleLike = async () => {
    ...
    // Optimistically flip the UI immediately for responsiveness
    ...
    // Re-read the real count from the DB and sync like_count column
    const countRes = await dbSelect("fan_art_likes", `art_id=eq.${postId}&select=id`);
    const realCount = countRes.ok ? countRes.data.length : null;
    if (realCount !== null) {
      await dbUpdate("fan_art", `id=eq.${postId}`, { like_count: realCount });
