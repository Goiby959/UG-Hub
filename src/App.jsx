...
// Manual badges come straight from user_badges. Automatic badges
// (admin / hall_of_fame / fan_art_likes) are computed live from
// existing data, so they're always in sync and never go stale.
async function getUserBadges(userId, userRole) {
  ...
  const likeBadges = allBadges.filter((b) => b.kind === "fan_art_likes");
  if (likeBadges.length > 0) {
    const faRes = await dbSelect("fan_art", "user_id=eq." + userId + "&select=like_count");
    const total = faRes.ok ? faRes.data.reduce((sum, r) => sum + (r.like_count || 0), 0) : 0;
    likeBadges.forEach((b) => { if (total >= (b.threshold || 0)) earned.push(b); });
  }
      ...
      {route.name === "home" ? (
        ...
        <WikiShell>
          {route.name === "wiki" ? (
            ...
            <BadgesDirectory isAdmin={isAdmin} onNavigate={navigate} />
          ) : route.name === "badges-manage" ? (
            isAdmin ? (
              <ManageBadges onNavigate={navigate} showToast={showToast} />
            ) : (
              <NotAllowed onBack={() => navigate({ name: "badges" })} />
            )
  ...
  return (
    <>
      ...
      >
        ...
        <MenuLink label="Badges" icon={<Icon.Star />} onClick={() => onNavigate({ name: "badges" })} />
        {isAdmin && <MenuLink label="Admin · Manage Users" icon={<Icon.Shield color={tokens.amber} />} onClick={() => onNavigate({ name: "admin" })} />}
        {isAdmin && <MenuLink label="Admin · Hall of Fame" icon={<Icon.Trophy color={tokens.amber} />} onClick={() => onNavigate({ name: "hall-of-fame-manage" })} />}
        {isAdmin && <MenuLink label="Admin · Badges" icon={<Icon.Star color={tokens.amber} />} onClick={() => onNavigate({ name: "badges-manage" })} />}
...
function HallOfFameGallery({ isAdmin, onNavigate }) {
  ...
  useEffect(() => {
    (async () => {
      const res = await dbSelect("hall_of_fame", "select=id,name,title,photo_url&order=display_order.asc");
      ...
      {entries.length === 0 ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          {entries.map((e) => (
            ...
            >
              <div style={{ background: "linear-gradient(135deg, #2F2212 0%, " + tokens.surface + " 100%)", border: "1px solid " + tokens.border, borderRadius: 10, overflow: "hidden" }}>
                <div style={{ width: "100%", aspectRatio: "1/1", overflow: "hidden", background: tokens.peat }}>
                  <img src={e.photo_url} alt={e.name} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                </div>
                <div style={{ padding: "10px 12px" }}>
                  <div style={{ fontWeight: 700, fontSize: "0.88rem", color: tokens.cream, overflowWrap: "anywhere", marginBottom: e.title ? 4 : 0 }}>
                    {e.name}
                  </div>
                  {e.title && (
                    <div style={{ fontSize: "0.72rem", color: tokens.amber, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {e.title}
                    </div>
                  )}
                </div>
...
function HallOfFameEntryView({ entryId, currentUser, isAdmin, onBack, onEdit, onDeleted, onNavigate, showToast }) {
  ...
  return (
    <div>
      ...
      <div style={{ width: "100%", maxWidth: 320, margin: "0 auto 20px", aspectRatio: "1/1", borderRadius: 12, overflow: "hidden", border: `2px solid ${tokens.borderStrong}`, background: tokens.peat }}>
        <img src={entry.photo_url} alt={entry.name} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
      </div>
...
function HallOfFameManage({ onNavigate, showToast }) {
  ...
  const load = useCallback(async () => {
    const res = await dbSelect("hall_of_fame", "select=id,name,title,photo_url,display_order&order=display_order.asc");
    setEntries(res.ok ? res.data : []);
    setLoading(false);
  }, []);
  ...
  const move = async (index, direction) => {
    ...
    const [resA, resB] = await Promise.all([
      dbUpdate("hall_of_fame", "id=eq." + a.id, { display_order: b.display_order }),
      dbUpdate("hall_of_fame", "id=eq." + b.id, { display_order: a.display_order }),
    ]);
    setBusyId(null);
    if (!resA.ok || !resB.ok) { showToast("Couldn't reorder", "error"); return; }
    const next = [...entries];
    next[index] = { ...b, display_order: a.display_order };
    next[newIndex] = { ...a, display_order: b.display_order };
    setEntries(next);
  };
  ...
  return (
    <div>
      <button onClick={() => onNavigate({ name: "hall-of-fame" })} className="ughub-btn-ghost" style={{ ...buttonStyles.ghostSmall, marginBottom: 20 }}>← Hall of Fame</button>
      ...
      {entries.length === 0 ? (
        ...
        <div style={{ display: "grid", gap: 8 }}>
          {entries.map((e, i) => (
            <div key={e.id} style={{ display: "flex", alignItems: "center", gap: 12, background: tokens.surface, border: `1px solid ${tokens.border}`, borderRadius: 8, padding: "10px 12px" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                <button onClick={() => move(i, -1)} disabled={i === 0 || busyId === e.id} className="ughub-btn-ghost" style={{ ...buttonStyles.ghostSmall, padding: 4, opacity: i === 0 ? 0.3 : 1 }}>
                  <Icon.ArrowUp />
                </button>
                <button onClick={() => move(i, 1)} disabled={i === entries.length - 1 || busyId === e.id} className="ughub-btn-ghost" style={{ ...buttonStyles.ghostSmall, padding: 4, opacity: i === entries.length - 1 ? 0.3 : 1 }}>
                  <Icon.ArrowDown />
                </button>
              </div>

              <div style={{ width: 44, height: 44, borderRadius: 8, overflow: "hidden", flexShrink: 0, background: tokens.peat }}>
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                ...
                {e.title && <div style={{ fontSize: "0.72rem", color: tokens.amber, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{e.title}</div>}
              </div>
...
/* ════════════════════════════════════════════════════════════
   BADGES (community recognition — some earned automatically,
   some awarded manually by admins)
   ════════════════════════════════════════════════════════════ */

function BadgesDirectory({ isAdmin, onNavigate }) {
  ...
  const howToEarn = (b) => {
    if (b.kind === "fan_art_likes") return `Reach ${b.threshold} total likes across your Fan Art posts.`;
    if (b.kind === "hall_of_fame") return "Be linked to a Hall of Fame entry.";
    if (b.kind === "admin") return "Be a member of the admin team.";
    return b.description || "Awarded by an admin.";
  };
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 16, gap: 12 }}>
        <div>
          <p style={{ fontFamily: "'Bebas Neue'", color: tokens.moss2, fontSize: "1rem", letterSpacing: "0.08em", textTransform: "uppercase", margin: "0 0 4px" }}>Community</p>
          <h1 style={{ fontFamily: "'Bebas Neue'", fontWeight: 900, fontSize: "2rem", color: tokens.cream, margin: 0 }}>Badges</h1>
        </div>
        {isAdmin && (
          <button onClick={() => onNavigate({ name: "badges-manage" })} className="ughub-btn-ghost" style={{ ...buttonStyles.ghostSmall, display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
            <Icon.Shield color={tokens.amber} /> Manage
          </button>
        )}
      </div>
      ...
      <p style={{ color: "rgba(217,200,168,0.55)", fontSize: "0.88rem", lineHeight: 1.5, margin: "0 0 20px" }}>
        Ways to earn recognition around UG Hub.
      </p>

      {badges.length === 0 ? (
        <div style={{ textAlign: "center", padding: "60px 0", color: "rgba(217,200,168,0.4)" }}>
          <Icon.Star color="rgba(217,200,168,0.2)" width={40} height={40} />
          <p style={{ marginTop: 16 }}>No badges set up yet.</p>
        </div>
      ) : (
        <div style={{ display: "grid", gap: 10 }}>
          {badges.map((b) => (
            <div key={b.id} style={{ background: tokens.surface, border: `1px solid ${tokens.border}`, borderRadius: 10, padding: "14px 16px" }}>
              <div style={{ marginBottom: 8 }}><BadgeChip badge={b} /></div>
              <p style={{ margin: 0, fontSize: "0.84rem", color: "rgba(217,200,168,0.6)", lineHeight: 1.5 }}>
                {howToEarn(b)}
              </p>
            </div>
          ))}
...
function ManageBadges({ onNavigate, showToast }) {
  ...
  return (
    <div>
      <button onClick={() => onNavigate({ name: "badges" })} className="ughub-btn-ghost" style={{ ...buttonStyles.ghostSmall, marginBottom: 20 }}>← Badges</button>
...
function BadgeEditor({ editId, onSaved, onCancel, showToast }) {
  ...
  const handleSave = async () => {
    if (!name.trim()) { window.alert("Please add a name."); return; }
    if (kind === "fan_art_likes" && (!threshold || isNaN(parseInt(threshold, 10)) || parseInt(threshold, 10) < 1)) {
      window.alert("Please enter a valid number of likes needed.");
      return;
    }
    setSaving(true);
    const payload = {
      ...
      color,
      kind,
      threshold: kind === "fan_art_likes" ? parseInt(threshold, 10) : null,
    };
    const res = isEditing
  ...
  return (
    <div>
      ...
      {kind === "fan_art_likes" && (
        <div style={{ marginBottom: 20 }}>
          <label style={{ display: "block", fontSize: "0.78rem", color: "rgba(217,200,168,0.55)", marginBottom: 4, letterSpacing: "0.04em" }}>LIKES NEEDED</label>
          <input
            type="number"
            inputMode="numeric"
            value={threshold}
  ...
  if (justSucceeded) {
    return (
      <div style={{ textAlign: "center", padding: "60px 20px" }}>
        ...
        <h1 style={{ fontFamily: "'Bebas Neue'", fontWeight: 900, fontSize: "1.6rem", color: tokens.cream, marginBottom: 8 }}>
          Thank you so much!
        </h1>
  ...
  return (
    <div>
      ...
      <div style={{ textAlign: "center", marginBottom: 28 }}>
        ...
        <h1 style={{ fontFamily: "'Bebas Neue'", fontWeight: 900, fontSize: "1.8rem", color: tokens.cream, margin: "0 0 8px" }}>
          Make a Donation
        </h1>
