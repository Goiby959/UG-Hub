      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: "grid", gap: 8, marginBottom: 16 }}>
          {comments.map((c) => {
            ...
            return (
              <div key={c.id} style={{ background: tokens.surface, border: `1px solid ${tokens.border}`, borderRadius: 8, padding: "10px 14px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8, marginBottom: 6 }}>
                  <span style={{ fontWeight: 700, fontSize: "0.82rem", color: tokens.amber }}>{c.username}</span>
                  {canDeleteComment && (
                    <button onClick={() => deleteComment(c.id)} style={{ background: "none", border: "none", padding: 0, cursor: "pointer", color: "rgba(217,200,168,0.3)", flexShrink: 0 }}>
