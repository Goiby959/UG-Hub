...
function htmlToPreviewText(html, maxLength = 110) {
  if (!html) return "";
  let text;
  try {
...
export default function UGHub() {
  ...
  const refreshPageIndex = useCallback(async () => {
    ...
    const res = await dbSelect("wiki_pages", "select=slug,title,is_private,updated_at,updated_by,header_image,content&order=updated_at.desc");
    if (res.ok) {
      setPageIndex(
        res.data.map((p) => ({
          slug: p.slug,
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 20 }}>
        ...
        <div style={{ flex: 1, minWidth: 0, position: "relative" }}>
          ...
          <input
            ...
            style={{ ...inputStyle, padding: "10px 14px 10px 36px", fontSize: "0.88rem" }}
          />
        </div>
      </div>
      ...
      <div style={{ position: "relative" }}>
        ...
        {popover && (
          <ToolbarPopover
            kind={popover}
            showDisplayTextField={popover === "link" && !hasTextSelection}
...
function ToolbarPopover({ kind, showDisplayTextField, onConfirm, onClose }) {
  ...
  const submit = () => {
    if (isLink) onConfirm(url, displayText);
    else onConfirm(url);
  };
