...
// Weighted sort — boosted and owner-liked posts bubble up with a random
// jitter so the order isn't completely deterministic every load.
function weightedSort(posts) {
  return [...posts].sort((a, b) => {
    const scoreA = (a.boost_count || 0) * 2 + (a.owner_liked ? 5 : 0) + Math.random() * 1.5;
    const scoreB = (b.boost_count || 0) * 2 + (b.owner_liked ? 5 : 0) + Math.random() * 1.5;
      ...
      {route.name === "home" ? (
        ...
        <WikiShell>
          {route.name === "wiki" ? (
            ...
            currentUser ? (
              <FanArtPost postId={route.postId} currentUser={currentUser} isBannedUser={isBannedUser} onBack={() => navigate({ name: "fan-art", ts: Date.now() })} showToast={showToast} />
            ) : (
              <NotAllowed onBack={() => navigate({ name: "wiki" })} />
            )
          ) : route.name === "fan-art-upload" ? (
            currentUser && !isBannedUser ? (
              <FanArtUpload currentUser={currentUser} onSaved={(id) => navigate({ name: "fan-art-post", postId: id })} onCancel={() => navigate({ name: "fan-art" })} showToast={showToast} />
            ) : (
  ...
  return (
    <style>{`
      ...
      * { box-sizing: border-box; }
      ...
      .ughub-art-card.owner-liked { border: 2px solid ${tokens.amber} !important; box-shadow: 0 0 12px rgba(201,138,46,0.35); }
      ...
      .ughub-art-card.boost-2 { border: 2px solid #5bafd4 !important; box-shadow: 0 0 10px rgba(91,175,212,0.55), 0 0 20px rgba(91,175,212,0.25); }

      /* Boost tier 3 (6-9 boosts): pulsing blue */
      .ughub-art-card.boost-3 { border: 2px solid #5bafd4 !important; box-shadow: 0 0 12px rgba(91,175,212,0.7), 0 0 28px rgba(91,175,212,0.35); animation: ughub-boost-pulse 2s ease-in-out infinite; }

      /* Boost tier 4 (10+ boosts): animated gradient border */
      .ughub-art-card.boost-4 { border: 2px solid transparent !important; background-clip: padding-box; position: relative; }
      .ughub-art-card.boost-4::before { content: ''; position: absolute; inset: -2px; border-radius: 12px; background: linear-gradient(270deg, #5bafd4, #a78bfa, #5bafd4, #38bdf8); background-size: 300% 300%; animation: ughub-boost-gradient 3s ease infinite; z-index: -1; }
      ...
      @keyframes ughub-boost-gradient {
...
function SideMenu({ open, onClose, currentUser, isAdmin, isOwner, isBannedUser, onNavigate, onLogout }) {
  return (
    <>
      <div
        onClick={onClose}
      ...
      >
        ...
        {isAdmin && <MenuLink label="Admin · Manage Users" icon={<Icon.Shield color={tokens.amber} />} onClick={() => onNavigate({ name: "admin" })} />}
        ...
        <div style={{ marginTop: "auto" }}>
          ...
          {currentUser ? (
            <div style={{ padding: "10px 24px 4px" }}>
              ...
              <button onClick={() => { onNavigate({ name: "profile" }); }} style={{ ...buttonStyles.ghostSmall, width: "100%", marginBottom: 6, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
        <button onClick={onBack} style={buttonStyles.ghostSmall}>
          ← Back to Wiki
        </button>
        {isAdmin && (
...
function BanControls({ u, onBan, onUnban }) {
  ...
  return (
    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
      ...
      <button
        onClick={handleBan}
        style={{ ...buttonStyles.ghostSmall, color: tokens.danger, borderColor: "rgba(194,74,58,0.35)", fontSize: "0.78rem" }}
      >
        Ban
      </button>
      <button
...
function AdminView({ currentUser, showToast, onNavigate }) {
  ...
  return (
    <div>
      <p style={{ fontFamily: "'Bebas Neue'", color: tokens.moss2, fontSize: "1rem", letterSpacing: "0.18em", textTransform: "uppercase", marginBottom: 4 }}>
        Admin Tools
      </p>
      <h1 style={{ fontFamily: "'Rubik'", fontWeight: 900, fontSize: "2rem", color: tokens.cream, marginTop: 0, marginBottom: 16 }}>
        Manage Users
      </h1>
      ...
      {admins.length > 0 && (
        <section style={{ marginBottom: 28 }}>
          <p style={{ fontFamily: "'Bebas Neue'", color: tokens.amber, fontSize: "0.85rem", letterSpacing: "0.14em", textTransform: "uppercase", margin: "0 0 10px" }}>
            Admins
          </p>
      ...
      <section>
        <p style={{ fontFamily: "'Bebas Neue'", color: "rgba(217,200,168,0.5)", fontSize: "0.85rem", letterSpacing: "0.14em", textTransform: "uppercase", margin: "0 0 10px" }}>
          Viewers
        </p>
        {viewers.length === 0 ? (
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
        ...
        {isOwner && !isBanned && (
          <button
            ...
            style={{ ...buttonStyles.ghostSmall, color: tokens.amber, borderColor: "rgba(201,138,46,0.35)" }}
          >
            Make Admin
          </button>
        )}
        {isBanned && (
          <button onClick={onUnban} style={{ ...buttonStyles.ghostSmall, fontSize: "0.78rem" }}>
            Unban
          </button>
        )}
      </div>
        ...
        <div style={{ flex: 1, display: "flex", gap: 8 }}>
          ...
          {headerImage && (
            <button
              ...
              style={{ ...buttonStyles.ghostSmall, padding: "10px 12px", color: tokens.danger, borderColor: "rgba(194,74,58,0.4)" }}
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
        <button onClick={onClose} style={{ ...buttonStyles.ghostSmall, padding: "8px 12px", fontSize: "0.82rem" }}>
          <Icon.X width={14} height={14} />
        </button>
        <button onClick={submit} style={{ ...buttonStyles.primarySmall, padding: "8px 16px", fontSize: "0.82rem" }}>
          Add
        </button>
      </div>
    </div>
...
function FanArtGallery({ currentUser, isBannedUser, onNavigate, showToast }) {
  ...
  useEffect(() => {
    (async () => {
      const [postsRes, likesRes, boostsRes] = await Promise.all([
        dbSelect("fan_art", "select=id,title,image_url,username,created_at,owner_liked&order=created_at.desc"),
        dbSelect("fan_art_likes", "select=art_id"),
        dbSelect("fan_art_boosts", "select=art_id"),
      ]);
      if (postsRes.ok) {
        const likeMap = {};
        if (likesRes.ok) likesRes.data.forEach((l) => { likeMap[l.art_id] = (likeMap[l.art_id] || 0) + 1; });
        const boostMap = {};
        if (boostsRes.ok) boostsRes.data.forEach((b) => { boostMap[b.art_id] = (boostMap[b.art_id] || 0) + 1; });
        const withCounts = postsRes.data.map((p) => ({
          ...p,
          like_count: likeMap[p.id] || 0,
          boost_count: boostMap[p.id] || 0,
        }));
        setPosts(weightedSort(withCounts));
      }
      setLoading(false);
    })();
  }, []);

  // Tap outside any card to clear focus
  useEffect(() => {
    if (!focusedId) return;
    const handler = (e) => {
      if (!e.target.closest(".ughub-art-card")) setFocusedId(null);
    };
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 20 }}>
        <div>
          <p style={{ fontFamily: "'Bebas Neue'", color: tokens.moss2, fontSize: "1rem", letterSpacing: "0.18em", textTransform: "uppercase", margin: "0 0 4px" }}>Community</p>
          <h1 style={{ fontFamily: "'Rubik'", fontWeight: 900, fontSize: "2rem", color: tokens.cream, margin: 0 }}>Fan Art</h1>
        </div>
        {!isBannedUser && (
          <button onClick={() => onNavigate({ name: "fan-art-upload" })} style={{ ...buttonStyles.primarySmall, display: "flex", alignItems: "center", gap: 6, padding: "10px 16px" }}>
            <Icon.Image /> Upload
          </button>
        )}
      </div>
      ...
      {posts.length === 0 ? (
        <div style={{ textAlign: "center", padding: "60px 0", color: "rgba(217,200,168,0.4)" }}>
          ...
          <p style={{ marginTop: 16 }}>No fan art yet — be the first to upload!</p>
        </div>
      ) : (
          ...
          {posts.map((p) => {
            ...
            return (
              <button
                ...
                onClick={(e) => {
                  if (focusedId === p.id) {
                    onNavigate({ name: "fan-art-post", postId: p.id });
                  } else {
                    e.stopPropagation();
                    setFocusedId(p.id);
                  }
                ...
                >
                  ...
                  <div style={{ padding: "10px 12px" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <span style={{ fontSize: "0.72rem", color: "rgba(217,200,168,0.45)" }}>{p.username}</span>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        {p.boost_count > 0 && (
                          <span style={{ display: "flex", alignItems: "center", gap: 3, fontSize: "0.68rem", color: tokens.sky }}>
                            ↑ {p.boost_count}
                          </span>
                        )}
                    ...
                    {isFocused && (
                      <div style={{ marginTop: 8, fontSize: "0.72rem", color: tokens.moss2, textAlign: "center", fontWeight: 600 }}>
                        Tap again to open →
                      </div>
                    )}
                  </div>
...
function FanArtPost({ postId, currentUser, isBannedUser, onBack, showToast }) {
  ...
  const [boostsRemaining, setBoostsRemaining] = useState(null); // null = not loaded yet
  ...
  useEffect(() => {
    (async () => {
      const [postRes, likeRes, allLikesRes, commentRes, boostRes, allBoostsRes, balanceRes] = await Promise.all([
        ...
        dbSelect("fan_art_likes", `art_id=eq.${postId}&select=id`),
        dbSelect("fan_art_comments", `art_id=eq.${postId}&select=*&order=created_at.asc`),
        dbSelect("fan_art_boosts", `art_id=eq.${postId}&user_id=eq.${currentUser.id}&select=id`),
        dbSelect("fan_art_boosts", `art_id=eq.${postId}&select=id`),
      ...
      setBoosted(boostRes.ok && boostRes.data.length > 0);
      setBoostCount(allBoostsRes.ok ? allBoostsRes.data.length : (p?.boost_count || 0));
      // Check if weekly balance needs resetting
      if (balanceRes.ok && balanceRes.data.length > 0) {
        const bal = balanceRes.data[0];
        const weekStart = new Date(bal.week_start);
        const now = new Date();
      ...
      } else if (balanceRes.ok && balanceRes.data.length === 0) {
        // No balance row yet — user has no premium, show 0
        setBoostsRemaining(0);
      }
      setComments(commentRes.ok ? commentRes.data : []);
      setLoading(false);
    })();
  }, [postId]);
  ...
  const toggleBoost = async () => {
    if (isBannedUser) return;
    if (boosted) {
      // Unboost — give the boost back
      await dbDelete("fan_art_boosts", `art_id=eq.${postId}&user_id=eq.${currentUser.id}`);
      setBoosted(false);
      setBoostCount((c) => Math.max(0, c - 1));
      if (!isUnlimitedBoosts && boostsRemaining !== null) {
        const newBal = (boostsRemaining || 0) + 1;
        await dbUpdate("user_boosts", `user_id=eq.${currentUser.id}`, { boosts_remaining: newBal });
        setBoostsRemaining(newBal);
      }
      const countRes = await dbSelect("fan_art_boosts", `art_id=eq.${postId}&select=id`);
      if (countRes.ok) {
        const real = countRes.data.length;
        setBoostCount(real);
        await dbUpdate("fan_art", `id=eq.${postId}`, { boost_count: real });
      }
    } else {
      // Can we boost?
      if (!isUnlimitedBoosts && (boostsRemaining === null || boostsRemaining <= 0)) {
        showToast("No boosts remaining this week", "error");
        return;
      }
      await dbInsert("fan_art_boosts", { art_id: postId, user_id: currentUser.id });
      setBoosted(true);
      setBoostCount((c) => c + 1);
      if (!isUnlimitedBoosts) {
        const newBal = (boostsRemaining || 0) - 1;
  ...
  const canBoost = isUnlimitedBoosts || (boostsRemaining !== null && boostsRemaining > 0) || boosted;
  ...
  const submitComment = async () => {
    ...
    setComments((prev) => [...prev, { ...res.data[0], username: currentUser.username, body: commentBody.trim(), created_at: new Date().toISOString() }]);
    setCommentBody("");
  };
  ...
  const saveEdit = async (commentId) => {
    ...
    const res = await dbUpdate("fan_art_comments", `id=eq.${commentId}`, { body: editingBody.trim() });
    if (!res.ok) { showToast("Couldn't update comment", "error"); return; }
    setComments((prev) => prev.map((c) => c.id === commentId ? { ...c, body: editingBody.trim() } : c));
  ...
  if (!post) return (
    <div>
      <button onClick={onBack} style={{ ...buttonStyles.ghostSmall, marginBottom: 20 }}>← Back</button>
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <button onClick={onBack} style={buttonStyles.ghostSmall}>← Fan Art</button>
        {canDelete && (
          <button onClick={deletePost} style={{ ...buttonStyles.ghostSmall, color: tokens.danger, borderColor: "rgba(194,74,58,0.35)", display: "flex", alignItems: "center", gap: 6 }}>
            <Icon.Trash /> Delete
          </button>
        )}
      </div>
      ...
      <h1 style={{ fontFamily: "'Rubik'", fontWeight: 900, fontSize: "1.6rem", color: tokens.cream, margin: "0 0 6px", overflowWrap: "anywhere" }}>
        {post.title}
      </h1>
      <p style={{ fontSize: "0.82rem", color: "rgba(217,200,168,0.5)", margin: "0 0 12px" }}>by {post.username}</p>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 28, flexWrap: "wrap" }}>
        ...
        <button
          onClick={toggleBoost}
          disabled={isBannedUser || (!canBoost && !boosted)}
          title={isUnlimitedBoosts ? "Unlimited boosts" : boostsRemaining === null ? "No boost balance" : (boostsRemaining + " boost" + (boostsRemaining !== 1 ? "s" : "") + " remaining this week")}
          style={{
            ...buttonStyles.ghostSmall,
            display: "flex", alignItems: "center", gap: 6,
            color: boosted ? tokens.sky : (canBoost ? tokens.bone : "rgba(217,200,168,0.3)"),
            borderColor: boosted ? "rgba(91,175,212,0.6)" : (canBoost ? tokens.border : "rgba(217,200,168,0.15)"),
            background: boosted ? "rgba(91,175,212,0.12)" : "transparent",
          }}
        >
          <span style={{ fontSize: "1rem", lineHeight: 1 }}>↑</span>
          {boostCount > 0 ? boostCount : "Boost"}
          {!isUnlimitedBoosts && boostsRemaining !== null && !boosted && boostsRemaining > 0 && (
        ...
        <button onClick={share} style={{ ...buttonStyles.ghostSmall, display: "flex", alignItems: "center", gap: 6 }}>
      ...
      <div style={{ marginBottom: 16 }}>
        <p style={{ fontFamily: "'Bebas Neue'", color: "rgba(217,200,168,0.5)", fontSize: "0.85rem", letterSpacing: "0.14em", textTransform: "uppercase", margin: "0 0 12px" }}>
        ...
        <div style={{ display: "grid", gap: 8, marginBottom: 16 }}>
          {comments.map((c) => {
            ...
            return (
              <div key={c.id} style={{ background: tokens.surface, border: `1px solid ${tokens.border}`, borderRadius: 8, padding: "10px 14px", position: "relative" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8, marginBottom: 6 }}>
                  <span style={{ fontWeight: 700, fontSize: "0.82rem", color: tokens.amber }}>{c.username}</span>
                  {canManage && (
                ...
                {isEditing ? (
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    <textarea
                      ...
                      onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); saveEdit(c.id); } if (e.key === "Escape") { setEditingCommentId(null); } }}
                      maxLength={500}
                      rows={3}
                      autoFocus
                      style={{ ...inputStyle, width: "100%", resize: "none", fontFamily: "'Rubik'", lineHeight: 1.5, fontSize: "0.88rem" }}
                    />
                    <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
                      <button onClick={() => setEditingCommentId(null)} style={{ ...buttonStyles.ghostSmall, fontSize: "0.78rem", padding: "6px 10px" }}>Cancel</button>
                      <button onClick={() => saveEdit(c.id)} disabled={!editingBody.trim()} style={{ ...buttonStyles.primarySmall, fontSize: "0.78rem", padding: "6px 12px" }}>Save</button>
                    </div>
                  </div>
                ) : (
                  <p style={{ margin: 0, fontSize: "0.88rem", color: tokens.bone, lineHeight: 1.5, overflowWrap: "anywhere" }}>{c.body}</p>
                )}
              </div>
            );
          })}
        ...
        {!isBannedUser && (
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button
              ...
              style={{ ...buttonStyles.primarySmall, padding: "10px 16px", flexShrink: 0 }}
            >
...
function FanArtUpload({ currentUser, onSaved, onCancel, showToast }) {
  const [title, setTitle] = useState("");
  ...
  return (
    <div>
      <button onClick={onCancel} style={{ ...buttonStyles.ghostSmall, marginBottom: 20 }}>← Cancel</button>
      <p style={{ fontFamily: "'Bebas Neue'", color: tokens.moss2, fontSize: "1rem", letterSpacing: "0.18em", textTransform: "uppercase", margin: "0 0 4px" }}>
        Community
      </p>
      <h1 style={{ fontFamily: "'Rubik'", fontWeight: 900, fontSize: "2rem", color: tokens.cream, margin: "0 0 24px" }}>
        Upload Fan Art
      </h1>
      ...
      {!imageUrl && !uploading && (
        <button onClick={() => fileRef.current?.click()} style={{ ...buttonStyles.ghostSmall, width: "100%", marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Name your artwork…" maxLength={80} style={{ ...inputStyle, width: "100%" }} />
      </div>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <textarea
          ...
          maxLength={500}
          rows={3}
          style={{ ...inputStyle, width: "100%", resize: "vertical", fontFamily: "'Rubik'", lineHeight: 1.5 }}
        />
      </div>

      <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
        <button onClick={onCancel} style={buttonStyles.ghostSmall}>Cancel</button>
        <button onClick={handleSave} disabled={saving || !imageUrl || !title.trim()} style={{ ...buttonStyles.primarySmall, padding: "10px 24px" }}>
          {saving ? "Posting…" : "Post"}
        </button>
      </div>
    </div>
...
function UserProfileView({ userId, onBack }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setProfile(res.ok && res.data.length > 0 ? res.data[0] : null);
      setLoading(false);
    })();
  }, [userId]);
      ...
      <div style={{ display: "flex", gap: 16, alignItems: "flex-start", marginBottom: 24 }}>
        ...
        <div style={{ minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            <h1 style={{ fontFamily: "'Rubik'", fontWeight: 900, fontSize: "1.6rem", color: tokens.cream, margin: 0, overflowWrap: "anywhere" }}>
...
function ProfileView({ currentUser, onSaved, showToast }) {
  ...
  return (
    <div>
      ...
      <h1 style={{ fontFamily: "'Rubik'", fontWeight: 900, fontSize: "2rem", color: tokens.cream, marginTop: 0, marginBottom: 24 }}>
        My Profile
      </h1>
      ...
      <div style={{ display: "flex", gap: 16, alignItems: "flex-start", marginBottom: 28 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8, flex: 1 }}>
          <button
            ...
            style={{ ...buttonStyles.ghostSmall, display: "flex", alignItems: "center", gap: 8, justifyContent: "center" }}
          >
          ...
          {avatarUrl && (
            <button
              ...
              style={{ ...buttonStyles.ghostSmall, color: tokens.danger, borderColor: "rgba(194,74,58,0.35)", display: "flex", alignItems: "center", gap: 6, justifyContent: "center" }}
            >
      ...
      <div style={{ display: "flex", justifyContent: "flex-end" }}>
        <button onClick={handleSave} disabled={saving} style={{ ...buttonStyles.primarySmall, padding: "10px 28px" }}>
