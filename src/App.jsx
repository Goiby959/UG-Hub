...
const tokens = {
  // Base surfaces — "candlelit ledger" instead of the old moss/peat theme
  peat: "#1C1712",        // deepest background
  soil: "#241D16",        // popover/modal background
  surface: "#2E2519",     // card/raised surface
  surfaceDeep: "#241D16", // recessed surface
  moss: "#57713F",        // primary accent (was bright green, now muted moss — success/nature)
  moss2: "#7A9B5C",       // brighter moss for hover/active states
  bone: "#E8DCC4",        // primary text (parchment)
  cream: "#F2E9D5",       // brightest text (headings)
  amber: "#D1A24A",       // brass — boosts/premium/gold-leaf highlight
  sky: "#7FB0C9",         // links/info (muted, less neon than before)
  danger: "#C4703F",      // rust — was pure red, now warm rust to match the palette
  border: "rgba(232,220,196,0.12)",
  borderStrong: "rgba(232,220,196,0.24)",
  // New tokens specific to the field-journal language
  rust: "#C4703F",        // primary accent — stamps, alerts, specimen tags
  ink: "#E8DCC4",         // alias of bone, used in new components for clarity
  inkFaint: "rgba(232,220,196,0.55)",
};
...
const Icon = {
  ...
  Stamp: (p) => (
    <svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...p}>
      ...
      <path d="M9 14.5 6.5 21l5.5-3 5.5 3-2.5-6.5" />
...
export default function UGHub() {
  ...
  const [route, setRoute] = useState(() => {
    ...
    // Returning from Stripe Checkout after a coffee donation
    if (hash === "coffee-success" || hash === "coffee-cancel") {
      return { name: "coffee" };
    }
    return { name: "home" };
  });
      ...
      {route.name === "home" ? (
        ...
        <WikiShell>
          {route.name === "wiki" ? (
            ...
            <CoffeeDonate
              currentUser={currentUser}
              onBack={() => navigate({ name: currentUser ? "wiki" : "home" })}
              showToast={showToast}
            />
...
function GlobalStyle() {
  return (
    <style>{`
      @import url('https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght@0,9..144,400;0,9..144,500;0,9..144,600;0,9..144,700;0,9..144,900;1,9..144,500&family=Inter:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500&display=swap');
...
function NotAllowed({ onBack }) {
  return (
    <div style={{ textAlign: "center", padding: "60px 0" }}>
      ...
      <h2 style={{ fontFamily: "'Inter'", marginTop: 16 }}>Admins only</h2>
    ...
    <>
      ...
      >
        ...
        >
          ...
          <span style={{ fontFamily: "'Inter'", fontWeight: 700, fontSize: "0.85rem", color: tokens.bone, letterSpacing: "0.04em", textTransform: "uppercase" }}>
            Home
          </span>
        </button>
        ...
        <div style={{ padding: "0 24px" }}>
          <button
            onClick={() => onNavigate({ name: "coffee" })}
          ...
          >
            <Icon.DollarSign /> Make a Donation
          </button>
        </div>
    ...
    <div style={{ position: "relative", minHeight: "100vh", overflow: "hidden" }}>
      ...
      >
        ...
        >
          ...
          <button
            onClick={() => onNavigate({ name: "coffee" })}
            style={{ ...ctaStyles.base, ...ctaStyles.outline, cursor: "pointer", color: tokens.amber, borderColor: "rgba(201,138,46,0.4)" }}
          >
            <Icon.Coffee width={22} height={22} />
            Make a Donation
          </button>
        </div>
      </main>
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <p style={{ fontFamily: "'JetBrains Mono'", color: tokens.moss2, fontSize: "1rem", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 4 }}>
          Knowledge Base
        </p>
        <h1 style={{ fontFamily: "'Fraunces'", fontWeight: 900, fontSize: "2.4rem", color: tokens.cream, margin: 0 }}>
          The Wiki
        </h1>
      </div>
    ...
    <div>
      ...
      {confirmDelete && (
        <Modal onClose={() => setConfirmDelete(false)}>
          <h3 style={{ fontFamily: "'Inter'", marginBottom: 10 }}>Delete "{page.title}"?</h3>
...
function LoginView({ onSuccess }) {
  ...
  return (
    <div style={{ maxWidth: 360, margin: "40px auto" }}>
      <h1 style={{ fontFamily: "'Fraunces'", fontWeight: 900, fontSize: "1.9rem", color: tokens.cream, textAlign: "center", marginBottom: 6 }}>
...
function AdminView({ currentUser, showToast, onNavigate }) {
  ...
  return (
    <div>
      <p style={{ fontFamily: "'JetBrains Mono'", color: tokens.moss2, fontSize: "1rem", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 4 }}>
        Admin Tools
      </p>
      <h1 style={{ fontFamily: "'Fraunces'", fontWeight: 900, fontSize: "2rem", color: tokens.cream, marginTop: 0, marginBottom: 16 }}>
        Manage Users
      </h1>
      ...
      {admins.length > 0 && (
        <section style={{ marginBottom: 28 }}>
          <p style={{ fontFamily: "'JetBrains Mono'", color: tokens.amber, fontSize: "0.85rem", letterSpacing: "0.08em", textTransform: "uppercase", margin: "0 0 10px" }}>
            Admins
          </p>
      ...
      <section>
        <p style={{ fontFamily: "'JetBrains Mono'", color: "rgba(217,200,168,0.5)", fontSize: "0.85rem", letterSpacing: "0.08em", textTransform: "uppercase", margin: "0 0 10px" }}>
          Viewers
        </p>
        {viewers.length === 0 ? (
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 16 }}>
        <div>
          <p style={{ fontFamily: "'JetBrains Mono'", color: tokens.moss2, fontSize: "1rem", letterSpacing: "0.08em", textTransform: "uppercase", margin: "0 0 4px" }}>Community</p>
          <h1 style={{ fontFamily: "'Fraunces'", fontWeight: 900, fontSize: "2rem", color: tokens.cream, margin: 0 }}>Fan Art</h1>
        </div>
        {!isBannedUser && (
            ...
            return (
              ...
              >
                <div className={cardClass} style={{ borderRadius: 10, transition: "transform 0.18s ease, box-shadow 0.18s ease, opacity 0.18s ease" }}>
                  <div style={{ background: "linear-gradient(135deg, #2F2212 0%, " + tokens.surface + " 100%)", border: "1px solid " + tokens.border, borderRadius: 9, overflow: "hidden", position: "relative" }}>
                    <span
                      style={{
                        position: "absolute",
                        top: 8,
                    ...
                    <div style={{ padding: "10px 12px" }}>
                      <div style={{ fontFamily: "'Fraunces'", fontWeight: 700, fontSize: "0.98rem", color: tokens.cream, overflowWrap: "anywhere", marginBottom: 4, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                        {p.title}
                      </div>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <button
                          onClick={(e) => { e.stopPropagation(); onNavigate({ name: "artist-profile", userId: p.user_id, username: p.username }); }}
                          style={{ background: "none", border: "none", padding: 0, cursor: "pointer", fontFamily: "'JetBrains Mono'", fontSize: "0.7rem", color: "rgba(217,200,168,0.45)", textDecoration: "underline", textDecorationColor: "rgba(217,200,168,0.2)" }}
                        >
                          @{p.username}
                        </button>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, fontFamily: "'JetBrains Mono'" }}>
                          {p.boost_count > 0 && <span style={{ fontSize: "0.68rem", color: tokens.amber }}>↑{p.boost_count}</span>}
                      ...
                      {isFocused && <div style={{ marginTop: 8, fontFamily: "'JetBrains Mono'", fontSize: "0.72rem", color: tokens.moss2, textAlign: "center", fontWeight: 600 }}>Tap again to open →</div>}
                    </div>
                  </div>
                </div>
              </button>
...
function FanArtPost({ postId, currentUser, isBannedUser, onBack, onNavigate, showToast }) {
  ...
  return (
    <div>
      ...
      <h1 style={{ fontFamily: "'Fraunces'", fontWeight: 900, fontSize: "1.6rem", color: tokens.cream, margin: "0 0 6px", overflowWrap: "anywhere" }}>
        {post.title}
      </h1>
      <button
      ...
      {isUnlimitedBoosts && boostsList.length > 0 && (
        <div style={{ marginBottom: 20 }}>
          <p style={{ fontFamily: "'JetBrains Mono'", color: "rgba(91,175,212,0.7)", fontSize: "0.82rem", letterSpacing: "0.08em", textTransform: "uppercase", margin: "0 0 8px" }}>
      ...
      <div style={{ marginBottom: 16 }}>
        <p style={{ fontFamily: "'JetBrains Mono'", color: "rgba(217,200,168,0.5)", fontSize: "0.85rem", letterSpacing: "0.08em", textTransform: "uppercase", margin: "0 0 12px" }}>
                ...
                {isEditing ? (
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    <textarea
                      ...
                      rows={3}
                      autoFocus
                      className="ughub-input" style={{ ...inputStyle, width: "100%", resize: "none", fontFamily: "'Inter'", lineHeight: 1.5, fontSize: "0.88rem" }}
                    />
                    <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
                      <button onClick={() => setEditingCommentId(null)} className="ughub-btn-ghost" style={{ ...buttonStyles.ghostSmall, fontSize: "0.78rem", padding: "6px 10px" }}>Cancel</button>
                      <button onClick={() => saveEdit(c.id)} disabled={!editingBody.trim()} className="ughub-btn-primary" style={{ ...buttonStyles.primarySmall, fontSize: "0.78rem", padding: "6px 12px" }}>Save</button>
                    </div>
                  </div>
                ) : (
        ...
        {!isBannedUser && (
          <div style={{ display: "flex", gap: 8 }}>
            <input
              ...
              maxLength={500}
              className="ughub-input" style={{ ...inputStyle, flex: 1 }}
            />
            <button
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 16, alignItems: "flex-start", marginBottom: 16 }}>
        ...
        <div style={{ minWidth: 0, flex: 1 }}>
          <h1 style={{ fontFamily: "'Fraunces'", fontWeight: 900, fontSize: "1.5rem", color: tokens.cream, margin: 0, overflowWrap: "anywhere" }}>
...
function FanArtUpload({ currentUser, editId, onSaved, onCancel, showToast }) {
  ...
  return (
    <div>
      ...
      <p style={{ fontFamily: "'JetBrains Mono'", color: tokens.moss2, fontSize: "1rem", letterSpacing: "0.08em", textTransform: "uppercase", margin: "0 0 4px" }}>
        Community
      </p>
      <h1 style={{ fontFamily: "'Fraunces'", fontWeight: 900, fontSize: "2rem", color: tokens.cream, margin: "0 0 24px" }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <textarea
          ...
          maxLength={500}
          rows={3}
          className="ughub-input" style={{ ...inputStyle, width: "100%", resize: "vertical", fontFamily: "'Inter'", lineHeight: 1.5 }}
        />
      </div>
      ...
      <div style={{ display: "flex", gap: 16, alignItems: "flex-start", marginBottom: 24 }}>
        ...
        <div style={{ minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            <h1 style={{ fontFamily: "'Fraunces'", fontWeight: 900, fontSize: "1.6rem", color: tokens.cream, margin: 0, overflowWrap: "anywhere" }}>
              {profile.display_name || profile.username}
            </h1>
...
function ProfileView({ currentUser, onSaved, showToast }) {
  ...
  return (
    <div>
      ...
      <h1 style={{ fontFamily: "'Fraunces'", fontWeight: 900, fontSize: "2rem", color: tokens.cream, marginTop: 0, marginBottom: 24 }}>
        My Profile
      </h1>
...
function CoffeeDonate({ currentUser, onBack, showToast }) {
  const [amount, setAmount] = useState("5");
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState("");

  // Read the return status from Stripe exactly once, on mount, and store
  // it in real state — reading window.location.hash directly on every
  // render breaks as soon as the hash is cleared (see effect below),
  // which made the thank-you screen flash and disappear immediately.
  const [returnStatus] = useState(() => {
    const hash = window.location.hash.slice(1);
    if (hash === "coffee-success") return "success";
    if (hash === "coffee-cancel") return "cancel";
    return null;
  });
  const justSucceeded = returnStatus === "success";
  const justCancelled = returnStatus === "cancel";

  useEffect(() => {
    // Clear the hash once we've read it, so refreshing doesn't re-trigger it
    if (returnStatus) {
  ...
  const handleDonate = async () => {
    ...
    try {
      ...
      // Read the raw text first so we can show it even if it isn't valid
      // JSON (e.g. Vercel returning an HTML 404 page instead of running
      // the function — this happens if the file path/name is wrong).
      const rawText = await res.text();
      let data;
      try {
        data = JSON.parse(rawText);
      } catch (parseErr) {
        setError("Server returned status " + res.status + ": " + rawText.slice(0, 200));
        setSubmitting(false);
        return;
      }
  ...
  if (justSucceeded) {
    return (
      <div style={{ textAlign: "center", padding: "60px 20px" }}>
        ...
        <h1 style={{ fontFamily: "'Fraunces'", fontWeight: 900, fontSize: "1.6rem", color: tokens.cream, marginBottom: 8 }}>
          Thank you so much!
        </h1>
        <p style={{ color: "rgba(217,200,168,0.6)", fontSize: "0.92rem", marginBottom: 24 }}>
          Your donation means a lot and helps keep UG Hub running. You're awesome.
        </p>
        <button onClick={onBack} className="ughub-btn-primary" style={{ ...buttonStyles.primarySmall, padding: "10px 28px" }}>
          Back to UG Hub
        </button>
      </div>
    );
  ...
  return (
    <div>
      ...
      <div style={{ textAlign: "center", marginBottom: 28 }}>
        ...
        <h1 style={{ fontFamily: "'Fraunces'", fontWeight: 900, fontSize: "1.8rem", color: tokens.cream, margin: "0 0 8px" }}>
          Make a Donation
        </h1>
        <p style={{ color: "rgba(217,200,168,0.55)", fontSize: "0.9rem", maxWidth: 340, margin: "0 auto" }}>
          UG Hub is a passion project. If you'd like to support it, any amount helps — thank you!
        </p>
      </div>

      {justCancelled && (
        <div style={{ background: "rgba(201,138,46,0.1)", border: "1px solid rgba(201,138,46,0.3)", borderRadius: 8, padding: "10px 14px", marginBottom: 20, fontSize: "0.84rem", color: tokens.amber, textAlign: "center" }}>
        ...
        {presets.map((p) => (
          <button
            ...
            style={{
              ...
              padding: "10px 18px",
              fontWeight: 700,
              color: Number(amount) === p ? tokens.peat : tokens.bone,
              background: Number(amount) === p ? tokens.amber : "transparent",
              borderColor: Number(amount) === p ? tokens.amber : tokens.border,
            }}
          >
            ${p}
          </button>
      ...
      <div style={{ maxWidth: 280, margin: "0 auto 8px" }}>
        <label style={{ display: "block", fontSize: "0.78rem", color: "rgba(217,200,168,0.55)", marginBottom: 6, textAlign: "center" }}>
          Or enter a custom amount
        </label>
        <div style={{ position: "relative" }}>
          <span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: "rgba(217,200,168,0.5)", fontSize: "1.1rem", fontWeight: 700 }}>$</span>
          <input
            type="number"
            min="1"
            max="1000"
            ...
            value={amount}
            onChange={(e) => setAmount(e.target.value)}
            className="ughub-input" style={{ ...inputStyle, width: "100%", paddingLeft: 28, textAlign: "center", fontSize: "1.2rem", fontWeight: 700 }}
          />
        </div>
        <div style={{ fontSize: "0.72rem", color: "rgba(217,200,168,0.35)", marginTop: 6, textAlign: "center" }}>
          Between $1 and $1000
        </div>
      </div>

      {error && (
        <p style={{ color: tokens.danger, fontSize: "0.84rem", textAlign: "center", marginBottom: 16, overflowWrap: "anywhere", padding: "0 12px" }}>{error}</p>
      )}
      ...
      <div style={{ maxWidth: 280, margin: "20px auto 0" }}>
        <button
          ...
          disabled={submitting}
          className="ughub-btn-primary" style={{ ...buttonStyles.primarySmall, width: "100%", padding: "12px 0", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}
        >
          {submitting ? <Spinner size={16} /> : <Icon.Coffee />}
          {submitting ? "Redirecting…" : "Donate $" + (parseFloat(amount) || 0).toFixed(2)}
        </button>
      </div>

      <p style={{ textAlign: "center", fontSize: "0.7rem", color: "rgba(217,200,168,0.3)", marginTop: 16 }}>
        Payments are securely processed by Stripe. UG Hub never sees your card details.
      </p>
    </div>
  );
}
