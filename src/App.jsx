...
export default function UGHub() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const res = await dbSelect("profiles", `id=eq.${currentSession.user.id}&select=id,username,role,banned_until`);
      if (res.ok && res.data.length > 0) {
        const profile = res.data[0];
        const bannedUntil = profile.banned_until ? new Date(profile.banned_until) : null;
        if (bannedUntil && bannedUntil > new Date()) {
          // Account is currently banned — sign them out silently.
          supabaseSignOut();
          setRestoringSession(false);
          return;
        }
...
function LoginView({ onSuccess }) {
  ...
  const fetchProfile = async (userId) => {
    const res = await dbSelect("profiles", `id=eq.${userId}&select=id,username,role,banned_until`);
    if (!res.ok || !res.data || res.data.length === 0) return null;
    return res.data[0];
  };

  const banMessage = (profile) => {
    if (!profile.banned_until) return null;
    const until = new Date(profile.banned_until);
    if (until <= new Date()) return null; // expired ban
    if (until.getFullYear() >= 2099) return "Your account has been permanently banned.";
    return `Your account is banned until ${until.toLocaleDateString(undefined, { month: "long", day: "numeric", year: "numeric" })}.`;
  };
...
function AdminView({ currentUser, showToast }) {
  ...
  const load = useCallback(async () => {
    const res = await dbSelect("profiles", "select=id,username,role,banned_until&order=username.asc");
  ...
  const isPerm = (u) => u.banned_until && new Date(u.banned_until).getFullYear() >= 2099;

  const banStatus = (u) => {
    if (!u.banned_until || new Date(u.banned_until) <= new Date()) return null;
    if (isPerm(u)) return "Permanently banned";
    const d = new Date(u.banned_until);
    return `Banned until ${d.toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })}`;
  };
  ...
  const setBan = async (u, kind) => {
    // kind: "temp" (7 days) | "perm" | "unban"
    ...
    const res = await dbUpdate("profiles", `id=eq.${u.id}`, { banned_until });
    if (!res.ok) { showToast("Couldn't update ban: " + res.error, "error"); return; }
    setUsers((prev) => prev.map((x) => (x.id === u.id ? { ...x, banned_until } : x)));
    const msg = kind === "unban" ? `${u.username} unbanned` : kind === "perm" ? `${u.username} permanently banned` : `${u.username} banned for 7 days`;
    showToast(msg, kind === "unban" ? "success" : "error");
  };
          ...
          <div style={{ display: "grid", gap: 8 }}>
            {viewers.map((u) => (
              <div
                key={u.id}
                style={{
                  background: isBanned(u) ? "rgba(194,74,58,0.07)" : tokens.surface,
                  border: `1px solid ${isBanned(u) ? "rgba(194,74,58,0.35)" : tokens.border}`,
                  borderRadius: 8,
                  padding: "12px 16px",
                  display: "flex",
                  flexDirection: "column",
              ...
              >
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
                    {isBanned(u) && <Icon.Lock color={tokens.danger} />}
                    <span style={{ fontWeight: 700, color: isBanned(u) ? "rgba(217,200,168,0.45)" : tokens.cream, overflowWrap: "anywhere" }}>
                      {u.username}
                    </span>
                  </div>
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap", flexShrink: 0 }}>
                    ...
                    {!isBanned(u) ? (
                      <>
                        <button onClick={() => setBan(u, "temp")} style={{ ...buttonStyles.ghostSmall, color: tokens.danger, borderColor: "rgba(194,74,58,0.35)", fontSize: "0.78rem" }}>
                          Ban 7d
                        </button>
                        <button onClick={() => setBan(u, "perm")} style={{ ...buttonStyles.ghostSmall, color: tokens.danger, borderColor: "rgba(194,74,58,0.35)", fontSize: "0.78rem" }}>
                          Ban ∞
                        </button>
                      </>
                    ) : (
                      <button onClick={() => setBan(u, "unban")} style={{ ...buttonStyles.ghostSmall, fontSize: "0.78rem" }}>
                        Unban
                      </button>
                    )}
                  </div>
                ...
                {banStatus(u) && (
                  <span style={{ fontSize: "0.74rem", color: tokens.danger }}>
                    {banStatus(u)}
                  </span>
                )}
              </div>
