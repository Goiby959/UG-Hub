...
function CoffeeDonate({ currentUser, onBack, showToast }) {
  ...
  // Check if we've just returned from Stripe with a success/cancel hash
  const hash = window.location.hash.slice(1);
  const justSucceeded = hash === "coffee-success";
  const justCancelled = hash === "coffee-cancel";

  useEffect(() => {
    // Clear the hash once we've read it, so refreshing doesn't re-trigger it
    if (justSucceeded || justCancelled) {
