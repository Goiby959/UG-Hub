...
export default function UGHub() {
  ...
  return (
    <div style={styles.appShell}>
      ...
      {/* Notification bell — fixed top left, present on every route */}
      <button
        onClick={() => {
          if (!currentUser) { navigate({ name: "login", returnTo: { name: "notifications" } }); return; }
          if (route.name === "notifications") {
            navigate(route.returnTo || { name: "wiki" });
          } else {
            navigate({ name: "notifications", returnTo: route });
          }
        }}
        aria-label={route.name === "notifications" ? "Close notifications" : "Notifications"}
        style={styles.notifBtn}
      >
        <Icon.Bell color={tokens.bone} width={20} height={20} />
        {unreadCount > 0 && (
      ...
      {route.name === "home" ? (
        ...
        <WikiShell transitionKey={JSON.stringify(route)}>
          {route.name === "wiki" ? (
            <HomeView
              pageIndex={pageIndex}
              isAdmin={isAdmin}
  ...
  return (
    <>
      ...
      >
        ...
        >
          <img src={ASSETS.logo} alt="UG logo" style={{ width: 90, height: "auto", filter: "drop-shadow(0 4px 14px rgba(74,124,63,0.45)) drop-shadow(0 4px 10px rgba(0,0,0,0.5))" }} draggable="false" />
      ...
      >
        ...
        <p
          style={{
            ...
            textAlign: "center",
            marginBottom: 48,
            animation: "ughub-fade-in 0.8s ease 1.2s both",
          }}
        >
          UG Hub is a fan-made project and is not affiliated with or endorsed by ContinuumXR.
        </p>
...
/* ════════════════════════════════════════════════════════════
   PARALLAX SHARDS — small low-poly triangulated shapes, scattered
   across every page, drifting at different speeds on scroll
   ════════════════════════════════════════════════════════════ */

// The three edge directions of an equilateral triangle grid (0°, 60°,
// 120° from horizontal) — every point below is reached by stepping
// along these, so every angle in every shape is a true 60°/120°.
const TRI_DIR = { A: [1, 0], B: [0.5, Math.sqrt(3) / 2], C: [-0.5, Math.sqrt(3) / 2] };
function triStep([x, y], dir, length) {
  const [dx, dy] = TRI_DIR[dir];
  return [x + dx * length, y + dy * length];
}
const pathFrom = (pts) => "M" + pts.map((p) => p[0].toFixed(1) + " " + p[1].toFixed(1)).join(" L");

// Three small motifs built off that grid: a closed triangle whose
// bottom vertex is a hub, branching out into 1-2 more limbs. Each is
// drawn as ONE continuous path — including retracing back through the
// hub to start a new limb — rather than separate <path> elements that
// only happen to share a coordinate. That was the cause of the visible
// break at the dots: two independent strokes near the same point,
// instead of one unbroken line running through it.
function LowPolyShard({ variant, color }) {
  if (variant === 0) {
    const p0 = [26, 10], p1 = triStep(p0, "A", 30), pHub = triStep(p0, "B", 30);
    const pRight = triStep(pHub, "A", 24);
    const pMid = triStep(pHub, "B", 22), pLeaf = triStep(pMid, "C", 22);
    const full = pathFrom([p0, p1, pHub, p0, pHub, pRight, pHub, pMid, pLeaf]);
    return (
      <svg viewBox="0 0 100 100" width="100%" height="100%">
        <path d={full} stroke={color} strokeWidth="4.5" fill="none" strokeLinejoin="round" strokeLinecap="round" />
        <line x1="34" y1="3" x2="48" y2="3" stroke="#fff" strokeWidth="3" strokeLinecap="round" opacity="0.9" />
        <line x1="47" y1="44" x2="59" y2="44" stroke="#fff" strokeWidth="3" strokeLinecap="round" opacity="0.9" />
        <circle cx={pLeaf[0]} cy={pLeaf[1]} r="7" fill={color} />
      </svg>
    );
  }
  if (variant === 1) {
    const p0 = [74, 10], p1 = triStep(p0, "A", -30), pHub = triStep(p0, "C", 30);
    const pLeft = triStep(pHub, "A", -24);
    const pMid = triStep(pHub, "C", 22), pLeaf = triStep(pMid, "B", 22);
    const full = pathFrom([p0, p1, pHub, p0, pHub, pLeft, pHub, pMid, pLeaf]);
    return (
      <svg viewBox="0 0 100 100" width="100%" height="100%">
        <path d={full} stroke={color} strokeWidth="4.5" fill="none" strokeLinejoin="round" strokeLinecap="round" />
        <line x1="52" y1="3" x2="66" y2="3" stroke="#fff" strokeWidth="3" strokeLinecap="round" opacity="0.9" />
        <line x1="53" y1="44" x2="41" y2="44" stroke="#fff" strokeWidth="3" strokeLinecap="round" opacity="0.9" />
        <circle cx={pLeaf[0]} cy={pLeaf[1]} r="7" fill={color} />
      </svg>
    );
  }
  const p0 = [16, 76], p1 = triStep(p0, "A", 30), pHub = triStep(p0, "C", -30);
  const pUp = triStep(pHub, "B", -24);
  const pZig = triStep(pUp, "A", 22), pLeaf = triStep(pZig, "C", -22);
  const full = pathFrom([p0, p1, pHub, p0, pHub, pUp, pZig, pLeaf]);
  return (
    <svg viewBox="0 0 100 100" width="100%" height="100%">
      <path d={full} stroke={color} strokeWidth="4.5" fill="none" strokeLinejoin="round" strokeLinecap="round" />
      <line x1="24" y1="83" x2="38" y2="83" stroke="#fff" strokeWidth="3" strokeLinecap="round" opacity="0.9" />
      <circle cx={pLeaf[0]} cy={pLeaf[1]} r="7" fill={color} />
    </svg>
  );
}

// Scatters small LowPolyShards across the viewport. Each one scrolls
// at its own speed, which is what creates the parallax illusion — and
// each lives in its own vertical band exactly one viewport tall, so
// its position wraps instead of drifting away for good. That's what
// guarantees they're always on screen no matter how far you scroll.
function ParallaxShards() {
  const [shards] = useState(() =>
    Array.from({ length: 20 }).map((_, i) => ({
      ...
      parallax: 0.06 + Math.random() * 0.32,
    }))
  );
  const refs = useRef([]);
  ...
  useEffect(() => {
    ...
    const apply = () => {
      ...
      shards.forEach((s, i) => {
        ...
        if (!el) return;
        const raw = s.bandFrac * vh - y * s.parallax;
        const wrapped = ((raw % vh) + vh) % vh;
        el.style.top = wrapped + "px";
  ...
  return (
    <>
      {shards.map((s, i) => (
        ...
        >
          <LowPolyShard variant={s.variant} color={tokens.moss2} />
        </div>
      ))}
    </>
  );
...
function WikiShell({ children, transitionKey }) {
  const [orbs, setOrbs] = useState([]);

  useEffect(() => {
    const colors = ["rgba(107,173,92,0.12)", "rgba(201,138,46,0.09)"];
  ...
  return (
    <div
      style={{
        ...
        overflow: "hidden",
        background:
          "radial-gradient(ellipse 90% 50% at 50% 0%, rgba(74,124,63,0.10) 0%, transparent 60%)," +
          "radial-gradient(ellipse 70% 45% at 100% 100%, rgba(201,138,46,0.06) 0%, transparent 65%)," +
          tokens.peat,
      }}
    >
      {orbs.map((o) => (
        <div
          ...
          style={{
            ...
            left: o.left + "vw",
            filter: "blur(36px)",
            animation: "ughub-orb-drift ease-in-out infinite",
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 880, margin: "0 auto", padding: "90px 20px 80px" }}>
        <div key={transitionKey} style={{ animation: "ughub-page-in 0.45s cubic-bezier(0.16, 1, 0.3, 1) both" }}>
          {children}
        </div>
      </div>
    </div>
...
function FanArtPost({ postId, currentUser, isBannedUser, onBack, onNavigate, showToast }) {
  ...
  // skipping usernames that don't match a real account). Goes through
  // an RPC that bypasses RLS server-side, same pattern as the Hall of
  // Fame personal-message feature, rather than a direct table insert.
  const notifyMentions = async (body, commentId) => {
    ...
    const results = await Promise.all(res.data.map((u) => dbRpc("create_mention_notification", {
      target_user_id: u.id,
      p_art_id: postId,
    ...
    if (failed) showToast("Comment posted, but notifying didn't work: " + failed.error, "error");
  };
