...
// Three small connected-line motifs. Two end in a dot at a vertex,
// one doesn't — only "some" meeting points get a dot.
function LowPolyShard({ variant, color }) {
  if (variant === 0) {
    return (
      <svg viewBox="0 0 100 100" width="100%" height="100%">
        <path d="M20 15 L58 12 L38 46 Z" stroke={color} strokeWidth="2.5" fill="none" strokeLinejoin="round" strokeLinecap="round" />
        <line x1="30" y1="21" x2="43" y2="34" stroke="#fff" strokeWidth="2" strokeLinecap="round" opacity="0.7" />
        <path d="M38 46 L36 76 L66 84" stroke={color} strokeWidth="2.5" fill="none" strokeLinecap="round" strokeLinejoin="round" />
        <circle cx="66" cy="84" r="5.5" fill={color} />
      </svg>
    );
  }
  if (variant === 1) {
    return (
      <svg viewBox="0 0 100 100" width="100%" height="100%">
        <path d="M8 32 L46 14 L40 56 L78 44" stroke={color} strokeWidth="2.5" fill="none" strokeLinecap="round" strokeLinejoin="round" />
        <line x1="17" y1="36" x2="31" y2="27" stroke="#fff" strokeWidth="2" strokeLinecap="round" opacity="0.7" />
        <path d="M40 56 L18 88" stroke={color} strokeWidth="2.5" fill="none" strokeLinecap="round" />
        <circle cx="18" cy="88" r="5.5" fill={color} />
      </svg>
    );
  }
  return (
    <svg viewBox="0 0 100 100" width="100%" height="100%">
      <path d="M12 72 L52 18 L84 68 Z" stroke={color} strokeWidth="2.5" fill="none" strokeLinejoin="round" strokeLinecap="round" />
      <line x1="57" y1="34" x2="71" y2="50" stroke="#fff" strokeWidth="2" strokeLinecap="round" opacity="0.65" />
    </svg>
  );
}

// Scatters small LowPolyShards across the viewport. Each one scrolls
// at its own speed (a fraction of real scroll), which is what actually
// creates the parallax illusion. Updates transforms directly via refs
// on scroll instead of React state, so this never forces the actual
// page content to re-render while scrolling.
function ParallaxShards() {
  const [shards] = useState(() =>
    Array.from({ length: 12 }).map((_, i) => {
      const parallax = 0.06 + Math.random() * 0.3;
      return {
        id: i,
        variant: i % 3,
        top: Math.random() * 100,
  ...
  useEffect(() => {
    ...
    const apply = () => {
      ...
      shards.forEach((s, i) => {
        ...
        if (el) el.style.transform = `translateY(${-y * s.parallax}px) rotate(${s.rotation}deg)`;
      });
      ticking = false;
    };
    const onScroll = () => {
