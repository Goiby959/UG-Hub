      ...
      {route.name === "home" ? (
        ...
        <WikiShell>
          {route.name === "wiki" ? (
            ...
            currentUser ? (
              <FanArtGallery currentUser={currentUser} isBannedUser={isBannedUser} onNavigate={navigate} showToast={showToast} />
            ) : (
              <NotAllowed onBack={() => navigate({ name: "wiki" })} />
            )
          ) : route.name === "fan-art-post" ? (
            currentUser ? (
              <FanArtPost postId={route.postId} currentUser={currentUser} isBannedUser={isBannedUser} onBack={() => navigate({ name: "fan-art" })} showToast={showToast} />
            ) : (
              <NotAllowed onBack={() => navigate({ name: "wiki" })} />
            )
  ...
  return (
    <>
      ...
      >
        ...
        {currentUser && !isBannedUser && <MenuLink label="Fan Art" icon={<Icon.Gallery />} onClick={() => onNavigate({ name: "fan-art" })} />}
...
function FanArtGallery({ currentUser, isBannedUser, onNavigate, showToast }) {
  ...
  useEffect(() => {
    (async () => {
      const res = await dbSelect("fan_art", "select=id,title,image_url,username,like_count,created_at&order=created_at.desc");
...
function FanArtPost({ postId, currentUser, isBannedUser, onBack, showToast }) {
  ...
  const toggleLike = async () => {
    ...
    if (liked) {
      ...
      await dbUpdate("fan_art", `id=eq.${postId}`, { like_count: likeCount - 1 });
      setLiked(false);
      setLikeCount((c) => c - 1);
    } else {
