  ...
  useEffect(() => {
    (async () => {
      const [postsRes, likesRes] = await Promise.all([
        dbSelect("fan_art", "select=id,title,image_url,username,created_at,boost_count,owner_liked&order=created_at.desc"),
      ...
      if (postsRes.ok) {
        ...
        if (likesRes.ok) {
          likesRes.data.forEach((l) => {
            countMap[l.art_id] = (countMap[l.art_id] || 0) + 1;
          });
        }
        const withCounts = postsRes.data.map((p) => ({ ...p, like_count: countMap[p.id] || 0 }));
