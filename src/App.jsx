  ...
  useEffect(() => {
    (async () => {
      const [postsRes, likesRes] = await Promise.all([
        dbSelect("fan_art", "select=id,title,image_url,username,created_at&order=created_at.desc"),
        dbSelect("fan_art_likes", "select=art_id"),
      ]);
      if (postsRes.ok) {
        // Count likes per post from the likes table directly — more accurate
        // than the like_count column which can get out of sync.
        const countMap = {};
        if (likesRes.ok) {
        ...
        setPosts(postsRes.data.map((p) => ({ ...p, like_count: countMap[p.id] || 0 })));
      }
      setLoading(false);
    })();
  }, []);
...
function FanArtPost({ postId, currentUser, isBannedUser, onBack, showToast }) {
  ...
  useEffect(() => {
    (async () => {
      const [postRes, likeRes, allLikesRes, commentRes] = await Promise.all([
      ...
      setPost(p);
      // Read real count from the likes table directly
  ...
  const toggleLike = async () => {
    ...
    // Sync the like_count column from the real count for consistency
